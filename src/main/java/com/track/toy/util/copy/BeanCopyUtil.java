 ...
 *
 * 该方法默认在target对象（copy到的对象）上使用以上注解，如需在source使用注解，在方法调用时请使用包含{@link CopyType}的重载方法，并设置为CopyType.SOURCE
 *
...
public class BeanCopyUtil {
    public static <S, T> T deepCopy(Class<T> targetClz, S source) {
        return deepCopy(targetClz, source, null, "");
    }

    public static <S, T> T deepCopy(Class<T> targetClz, S source, String key) {
        return deepCopy(targetClz, source, null, key);
    }

    public static <S, T> T deepCopy(Class<T> targetClz, S source, CopyType type, String key) {
        return deepCopy(targetClz, source, source, targetClz, new HashMap<>(), type == null ? true : type.flg(), key);
    }

    public static <S, T> T deepCopy(Class<T> targetClz, S source, CopyType type) {
        return deepCopy(targetClz, source, source, targetClz, new HashMap<>(), type == null ? true : type.flg(), "");
    }
    ...
    private static <S, T> T deepCopy(Class<T> targetClz, S source, Object sRoot, Class<?> tRootClz, Map<Object, Object> beans, boolean flg, String key) {
        try {
            ...
            deepCopy(target, source, sRoot, tRootClz, beans, flg, key);
            return target;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static <S, T> void deepCopy(T target, S source, Object sRoot, Class<?> tRootClz, Map<Object, Object> beans, boolean flg, String key) {
        try {
                    ...
                    try {
                        ...
                        if (List.class.isAssignableFrom(tFieldType)) {
                            ...
                            for (Object subSource : sourceList) {
                                targetList.add(deepCopy(actualClass, subSource, sRoot, tRootClz, beans, flg, key));
                            }
                        ...
                        if (Set.class.isAssignableFrom(tFieldType)) {
                            ...
                            for (Object subSource : sourceSet) {
                                targetSet.add(deepCopy(actualClass, subSource, sRoot, tRootClz, beans, flg, key));
                            }
                        ...
                        if (Map.class.isAssignableFrom(tFieldType)) {
                            ...
                            for (Map.Entry subEntry : (Set<Map.Entry>) sourceMap.entrySet()) {
                                Object subKey = deepCopy(actualClass, subEntry.getKey(), sRoot, tRootClz, beans, flg, key);
                                Object subValue = deepCopy(actualClassSecond, subEntry.getValue(), sRoot, tRootClz, beans, flg, key);
                        ...
                        // object
                        tField.set(target, deepCopy(sField.getType(), sValue, sRoot, tRootClz, beans, flg, key));
                    } catch (Exception e) {
                        continue;
                    }
                }
            } else {
                for (String key0 : sourceFields.keySet()) {
                    try {
                        Field sField = sourceFields.get(key0);
                        Field tField = targetFields.get(key0);
                        ...
                        //other field
                        CopyFiled copyFiled = getAnnotation(sField, CopyFiled.class, key);
                        Class<? extends CopyCustom> handlerClz = CopyCustom.DefaultCopyCustom.class;
                        if (copyFiled != null) {
                            String[] values = copyFiled.value();
                            if (values.length == 0) {
                            ...
                            tField = tRootClz.getDeclaredField(values[0]);
                            handlerClz = copyFiled.handler();
                        }
                        ...
                        // reference
                        if (getAnnotation(sField, CopyReference.class, key) != null) {
                            tField.set(target, sValue);
                            continue;
                        }
                        ...
                        // list
                        if (List.class.isAssignableFrom(tFieldType)) {
                            Class<?> actualClass = getActualClass(sField);
                            List sourceList = (List) sValue;
                            CopyList copyList = getAnnotation(sField, CopyList.class, key);
                            List targetList = copyList != null
                                    ? copyList.value().newInstance()
                                    : new ArrayList();
                            for (Object subSource : sourceList) {
                        ...
                        if (Set.class.isAssignableFrom(tFieldType)) {
                            Class<?> actualClass = getActualClass(sField);
                            Set sourceSet = (Set) sValue;
                            CopySet copySet = getAnnotation(sField, CopySet.class, key);
                            Set targetSet = copySet != null
                                    ? copySet.value().newInstance()
                                    : new HashSet();
                            for (Object subSource : sourceSet) {
                        ...
                        if (Map.class.isAssignableFrom(tFieldType)) {
                            ...
                            Class<?> actualClassSecond = getActualClassSecond(tField);
                            Map sourceMap = (Map) sValue;
                            CopyMap copyMap = getAnnotation(sField, CopyMap.class, key);
                            Map targetMap = copyMap != null
                                    ? copyMap.value().newInstance()
                                    : new HashMap();
                            for (Map.Entry subEntry : (Set<Map.Entry>) sourceMap.entrySet()) {
